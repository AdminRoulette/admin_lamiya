...
import {getCityList, getWarehouseList} from "@/http/ExternalApi/addressesAPI";

const NovaPostCity = ({
                          defaultNpCityList, input_REGEXP,
    ...
    const onClickCity = async (cityName) => {
        setNovaCityName(cityName.name)
        changeDeliveryData({cityRef: cityName.ref, warehouseRef: ""})
        showCityList()
        setInputCity("")
        setArrCity(null);
    };
    ...
    return (
        <div>
            ...
            <div onClick={() => showCityList()} className={classes.delivery_city_button}>
                {novaCityName ? <span>{novaCityName}</span>
                    : <span
                        className={classes.cityNameDefault}>{language ? 'Выберите населенный пункт' : 'Оберіть населений пункт'}</span>}
                <svg className={isShowCityList ? classes.rotated : ''} xmlns="http://www.w3.org/2000/svg" width="22"
            ...
            <div className={`${classes.delivery_dropDown_cont} ${isShowCityList ? classes.active : ''}`}>
                <div className={classes.delivery_dropDown_block}>
                    <div className={classes.delivery_dropDown_header}>
                        <svg onClick={() => showCityList()} xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                             viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="m12 19-7-7 7-7"/>
                            <path d="M19 12H5"/>
                        </svg>
                        <span>{language ? 'Населенный пункт' : 'Населений пункт'}</span>
                    </div>
                    ...
                    <ul className={classes.delivery_city_list}>
                        {arrCity?.length > 0 ?
                            ...
                            :
                            <>
                                {arrCity?.length === 0 && <div className={classes.empty_delivery_list}>
                                    <svg
                                    ...
                                    <span>{language ? 'Город не найден'
                                        : 'Місто не знайдено'}</span>
                                </div>}
                                ...
                                {defaultNpCityList.map((city, index) => {
