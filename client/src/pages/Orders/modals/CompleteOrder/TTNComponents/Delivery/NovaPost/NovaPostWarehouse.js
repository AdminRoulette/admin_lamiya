...
const NovaPostWarehouse = ({
                               language,
                               changeDeliveryData,
                               deliveryData
                           }) => {
    const [filterWarehouseArr, setFilterWarehouseArr] = useState([]);
    const [warehouseName, setWarehouseName] = useState("");
    const [inputWarehouse, setInputWarehouse] = useState("");
    const refWarehouseInput = useRef(null);
    const [warehouseList, setWarehouseList] = useState([]);
    const [loadingWarehouse, setLoadingWarehouse] = useState(false);
    const [isShowWarehouseList, setIsShowWarehouseList] = useState(false);

    useEffect(() => {
        setIsShowWarehouseList(false);
        setInputWarehouse("")
        setWarehouseName("")
        setLoadingWarehouse(true);
        getWarehouseList({cityRef:deliveryData.cityRef, language, provider:deliveryData.postMethod}).then((warehouseList) => {
    ...
    const onChangeWarehouse = async (warehouseName) => {
        setInputWarehouse(warehouseName.trim())
        if (warehouseName === "") {
            setFilterWarehouseArr([])
        } else {
            setFilterWarehouseArr(warehouseList.filter(item => item.name.toLowerCase().includes(warehouseName.trim().toLowerCase())))
        }
    };

    const onClickWarehouse = async (warehouse) => {
        setIsShowWarehouseList(prevState => !prevState);
        changeDeliveryData({warehouseRef:warehouse.warehouse_id})
        setWarehouseName(warehouse.name)
    };

    const showWarehouseList = () => {
        setIsShowWarehouseList(prevState => !prevState);
        refWarehouseInput.current.focus();
    };

    return (
        <>
            <div onClick={()=>setIsShowWarehouseList(false)}
                 className={`${classes.delivery_dropDown_bg} ${isShowWarehouseList ? classes.active : ""}`}/>
            <div>{language ? "Отделение" : "Відділення"}:</div>
            <div onClick={() => showWarehouseList()} className={classes.delivery_city_button}>
                {warehouseName ? <span>{warehouseName}</span>
                    : <span
                        className={classes.cityNameDefault}>{language ? 'Выберите отделение' : 'Оберіть відділення'}</span>}
                <svg className={isShowWarehouseList ? classes.rotated : ''} xmlns="http://www.w3.org/2000/svg" width="22" height="22"
                     viewBox="0 0 24 24"
                     fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="m6 9 6 6 6-6"/>
                </svg>
            </div>
            <div className={`${classes.delivery_dropDown_cont} ${isShowWarehouseList ? classes.active : ''}`}>
                <div className={classes.delivery_dropDown_block}>
                    <div className={classes.delivery_dropDown_header}>
                        <svg onClick={() => showWarehouseList()} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="m12 19-7-7 7-7"/>
                            <path d="M19 12H5"/>
                        </svg>
                        <span>{language ? 'Отделение' : 'Відділення'}</span>
                    </div>
                    <div className={classes.delivery_city_input}>
                        ...
                        <input ref={refWarehouseInput} type="text" onChange={(e) => onChangeWarehouse(e.target.value)}
                               value={inputWarehouse} maxLength={255}
                               placeholder={language ? 'Введите номер отделения' : 'Введіть номер відділення'}/>
                    </div>
                    <div
                        className={classes.delivery_gray_text}>{language
                        ? 'Отфильтруйте отделение по адресу или номеру'
                        : 'Відфільтруйте відділення за адресою чи номером'}
                    </div>
                    <div className={classes.delivery_loaded + (loadingWarehouse ? ` ${classes.loaded}` : "")}/>

                    <ul className={classes.delivery_city_list}>
                        {inputWarehouse.length > 0 && filterWarehouseArr.length > 0 || inputWarehouse.length === 0 && warehouseList.length > 0
                            ? inputWarehouse.length > 0 && filterWarehouseArr.length > 0
                                ? filterWarehouseArr.map((warehouse, index) => {
                                    return <li key={index} title={warehouse.name}
                                ...
                                <span>{language ? 'Отделение не найдено, проверьте корректность названия'
                                    : 'Відділення не знайдено, перевірте коректність назви'}</span>
                            </div>}
                    </ul>
                </div>
            </div>
...
export default NovaPostWarehouse;
